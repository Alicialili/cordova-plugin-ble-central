...
public class Peripheral extends BluetoothGattCallback {
    ...
    public final static String ACTION_DATA_AVAILABLE = Peripheral.class.getPackage().getName() + ".ACTION_DATA_AVAILABLE";//add by wanglili
    @Override
    public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        super.onCharacteristicChanged(gatt, characteristic);
        LOG.d(TAG, "onCharacteristicChanged " + characteristic);

        broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic); //broadcast the changed characteristic change. //add by wanglili
    ...
    public final static UUID UUID_MEASUREMENT_SERVICE = UUID.fromString(SampleGattAttributes.MEASUREMENT_SERVICE);
    public final static String EXTRA_DATA = Peripheral.class.getPackage().getName() + ".EXTRA_DATA";
    ...
    public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic); //broadcast each characteristic read result.//add by wanglili
        synchronized(this) {
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        synchronized(this) {
            if (writeCallback != null) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    //modify by Alicia on 20181204 start
                    //writeCallback.success();
                    PluginResult result = new PluginResult(PluginResult.Status.OK);
                    result.setKeepCallback(true);
                    writeCallback.sendPluginResult(result);
                    //modify by Alicia on 20181204 end
                } else {
                    writeCallback.error(status);
                }

                //writeCallback = null; //remove by Alicia on 20181204
            }
        }
