...
import com.mdbiomedical.app.mdcontroller.JniWrapper;
...
public class BLECentralPlugin extends CordovaPlugin implements BluetoothAdapter.LeScanCallback {
    ...
    public boolean execute(String action, CordovaArgs args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals(SCAN)) {
            ...
            JniWrapper.setInitPath(webView.getContext());
            addMyListener();
            //JniWrapper.reset();
    ...
    private void onBluetoothStateChange(Intent intent) {
        ...
        if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
            //add by wanglili start
            //final Intent intentt = new Intent("alicia.action");
            //webView.getContext().sendBroadcast(intentt);
            //add by wanglili end
    ...
    public static final byte BT_DOWNLOAD_LAST = 4;
    ...
    public static String dateString = DateFormat.getBestDateTimePattern(Locale.getDefault(), "yyyy/MM/dd HH:mm");
    public static java.text.DateFormat dateTimeInstance = new SimpleDateFormat(dateString, Locale.getDefault());
    ...
    private void release() {
        ...
        /*if (mBPMonitorService != null && mHeartRateCharacteristic != null) {
            data[1] = BT_STANDBY;
            mBPMonitorService.writeCharacteristic(mHeartRateCharacteristic, data);
        }*/
        //timer.cancel();
        /*if (isBound) {
        ...
        //if (mReceiverTag) {
            ...
            //mReceiverTag = false;
        //}
        //mBPMonitorService = null;
        LOG.i(TAG, "release resource");
    }
    //add by wanglili on 20181031 start
    private void sendDataToJs(CallbackContext callbackContext, int[] header){
        LOG.d(TAG, "start sendDataToJs method");
        JSONObject obj = new JSONObject();
        String datetime = "20" + header[2] + "/" + header[3] + "/" + header[4] + " " + header[5] + ":"
                + header[6] + ":" + header[7];
        ...
        header[10] = (header[10] & 0x00FF);
        header[11] = (header[11] & 0x00FF);
        header[13] = (header[13] & 0x00FF);
        try {
            obj.put("dateTime", dateTimeInstance.format(cc.getTime()));
            obj.put("pulse", String.valueOf(header[10]));
            obj.put("sys", String.valueOf(header[11] + header[12] * 256));
            obj.put("dia", String.valueOf(header[13] + header[14] * 256));
        } catch (JSONException e) {
            e.printStackTrace();
        }

        LOG.d(TAG, "sendDataToJs method: "+ obj.toString());
        PluginResult result = new PluginResult(PluginResult.Status.OK, obj);
        result.setKeepCallback(true);
        callbackContext.sendPluginResult(result);
    }
    private void onBluetoothMyChange(Intent intent) {
        final String action = intent.getAction();

        if (JniWrapper.path.equals(action)) {
            LOG.d(TAG, "JniWrapper_get send action");
            Bundle bundleJni = intent.getExtras();

            if (bundleJni != null) {
                String a = bundleJni.getString("name");
                byte[] senddata = new byte[20];
                if ("send".equals(a)) {
                    senddata = bundleJni.getByteArray("data");
                    if (senddata[1] != 0) {

                        //if (mHeartRateCharacteristic != null) {
                            try {
                                LOG.d(TAG, "JniWrapper_NDKSendData  try send ");
                                write(writeCallback, mMacAddress, Peripheral.UUID_MEASUREMENT_SERVICE, Peripheral.UUID_HEART_RATE_MEASUREMENT, senddata, BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT);
                                //mBPMonitorService.writeCharacteristic(mHeartRateCharacteristic, senddata);
                                //cmdflag = 1;
                                sec = 0;
                            } catch (Exception e) {
                                LOG.d(TAG, "JniWrapper_writeCharacteristic error ");
                            }
                        //}
                    }
                } else if ("addHeader".equals(a)) {
                    //cmdflag = 0;
                    int no = bundleJni.getInt("no");
                    // 每次add和取header，都是取272位，即16个数字
                    int[] header = bundleJni.getIntArray("header");
                    int x = bundleJni.getInt("x");
                    int y = bundleJni.getInt("y");
                    StringBuilder tt = new StringBuilder();
                    for (int d = 0; d < header.length; d++) {
                        tt.append(header[d] + ",");
                    }
                    LOG.d(TAG, "addheader= " + tt);
                    //0418
                    if (header[1] == 0 && header[10] == 0) {
                        totalCount = 0;
                        Toast.makeText(webView.getContext(), "no data", Toast.LENGTH_SHORT).show();
                        LOG.d(TAG, "current UserMode no data");

                    } else {
                        int[] header1 = new int[17];
                        for (int i = 0; i < header.length; i += 17) {
                            if (currentCount < totalCount) {
                                for (int j = 0; j < 17; j++) {
                                    header1[j] = header[j + i];
                                }
                                //addRecordView(mContext, no, header1, x, y);
                                sendDataToJs(writeCallback, header1);
                                currentCount++;
                            }
                        }
                        if (currentCount == totalCount) {
                            ...
                            //retryCmdCount = 0;
                            //Toast.makeText(webView.getContext(), "success" + currentCount, Toast.LENGTH_SHORT).show();
                        }
                    }
                }
            }
        }
        if (action.equals(Peripheral.ACTION_DATA_AVAILABLE)) {
            final int[] value = intent.getIntArrayExtra(Peripheral.EXTRA_DATA);
            String echo = JniWrapper.BLEToNDK(value);//不可以省略，用于发送命令
            sec = 0;
            seq = value[0];
            ...
            if (format == BT_DOWNLOAD) {
                if (bpCmd == BT_DOWNLOAD_U1_U2_COUNT) {
                    ...
                    LOG.e(TAG, "recordsCount1=" + recordsCount1 + ",recordsCount2=" + recordsCount2);
                    totalCount = (recordsCount1 + recordsCount2);
                    currentCount = 0;
                    sec = 0;
                    ...
                    if (totalCount == 0) {
                        //mHandler.removeCallbacks(mRunnable);
                        Toast.makeText(webView.getContext(), "Device no data", Toast.LENGTH_SHORT).show();
                        release();
                    }
                }else if (bpCmd == BT_DOWNLOAD_LAST) {
                  totalCount = 1;
                  currentCount = 0;
                }
            }        
    ...
    private static com.mdbiomedical.app.mdcontroller.JniWrapper JniWrapper = new JniWrapper();
    private void addMyListener() {
        if (this.myReceiver == null) {
            this.myReceiver = new BroadcastReceiver() {
                @Override
                public void onReceive(Context context, Intent intent) {
                    onBluetoothMyChange(intent);
                }
            };
        }
        ...
        try {
            ...
            intentFilter.addAction(Peripheral.ACTION_DATA_AVAILABLE);
            webView.getContext().registerReceiver(this.myReceiver, intentFilter);
        } catch (Exception e) {
            LOG.e(TAG, "Error registering my receiver: " + e.getMessage(), e);
        }
    }
    //add by wanglili end
